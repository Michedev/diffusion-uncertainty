...
class IPR():
    ...
    def precision_and_recall(self, subject):
        ...
        if isinstance(manifold_subject.features, np.ndarray) and isinstance(manifold_subject.radii, np.ndarray):
            manifold_subject = Manifold(torch.from_numpy(manifold_subject.features),
                                        torch.from_numpy(manifold_subject.radii))
    ...
    def compute_manifold_ref(self, path):
        self.manifold_ref: Manifold = self.compute_manifold(path)
    ...
    def extract_features(self, images):
        ...
        if height != 224 or width != 224:
            ...
            if height < 224 or width < 224:
                pad_h, pad_w = (224 - height) // 2, (224 - width) // 2
                pad_h_rest, pad_w_rest = (224 - height) % 2, (224 - width) % 2
                resize = partial(F.pad, pad=(pad_w, pad_w + pad_w_rest, pad_h, pad_h + pad_h_rest), mode='constant', value=0)
            else:
...
def compute_pairwise_distances(X, Y=None):
    ...
    Y = Y.astype(np.float64) if Y is not None else Y
...
class ImageFolder(Dataset):
    def __init__(self, root, transform=None):
        ...
        self.fnames = list(Path(root).walkfiles('*.png'))
