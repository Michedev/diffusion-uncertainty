...
def _compute_eigen_errors_visu(gt, pred, valid_mask=None):
...
def _compute_eigen_errors(gt, pred):
    """Computation of error metrics between predicted and ground truth depths
    """
    thresh = np.maximum((gt / pred), (pred / gt))
    a1 = (thresh < 1.25     ).mean()
...
def _compute_eigen_errors_v2(gt: np.ndarray, pred: np.ndarray, metrics=uncertainty_metrics, mask=None, reduce_mean=False):
...
@singledispatch
def compute_aucs(gt, pred, uncert, intervals):
    raise NotImplementedError("Unsupported type")

@compute_aucs.register
def _compute_aucs(gt: torch.Tensor, pred: torch.Tensor, uncert: torch.Tensor, intervals: int = 50):
    return compute_aucs(gt.cpu().numpy(), pred.cpu().numpy(), uncert.cpu().numpy(), intervals)

@compute_aucs.register
def _compute_aucs(gt: np.ndarray, pred: np.ndarray, uncert: np.ndarray, intervals: int = 50):
    ...
    uncert = -uncert
    true_uncert = _compute_eigen_errors_v2(gt ,pred)
    true_uncert = {metric: -true_uncert[i] for i, metric in enumerate(uncertainty_metrics)} #{"abs_rel" :-true_uncert[0] ,"rmse" :-true_uncert[1] ,"a1" :-true_uncert[2]}
    ...
    sparse_curve = \
        {m: [_compute_eigen_errors_v2(gt, pred, metrics=[m], mask=sub, reduce_mean=True)[0] for sub in subs] + [0] for m
    ...
    opt_curve = {m: [_compute_eigen_errors_v2(gt, pred, metrics=[m], mask=opt_sub, reduce_mean=True)[0] for opt_sub in
                     opt_subs[m]] + [0] for m in uncertainty_metrics}

    # compute metrics for random sampling (equal for each sampling)
    rnd_curve = {m: [_compute_eigen_errors_v2(gt, pred, metrics=[m], mask=None, reduce_mean=True)[0] for t in
